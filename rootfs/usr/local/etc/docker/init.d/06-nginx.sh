...
# @@Created          :  Friday, Nov 21, 2025 11:33 EST
# @@File             :  05-nginx.sh
# @@Description      :  
...
__execute_prerun() {
  ...
  if [ ! -f "$CONF_DIR/nginx.conf" ]; then
    mkdir -p "$CONF_DIR"
    local server_domain="${DOMAIN:-${HOSTNAME:-localhost}}"
    cat > "$CONF_DIR/nginx.conf" << NGINXEOF
...
http {
    ...
    # Collabora server (collabora.${DOMAIN})
    server {
        listen 80;
        server_name collabora.${server_domain};
    ...
    # Radicale server (radicale.${DOMAIN})
    server {
        listen 80;
        server_name radicale.${server_domain};
    ...
    # Default server (catch-all) - redirect to main domain
    server {
