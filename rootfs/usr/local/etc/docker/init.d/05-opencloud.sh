...
# @@License          :  WTFPL
# @@ReadME           :  01-opencloud.sh --help
# @@Copyright        :  Copyright: (c) 2025 Jason Hempstead, Casjays Developments
# @@Created          :  Friday, Nov 21, 2025 10:56 EST
# @@File             :  01-opencloud.sh
# @@Description      :  
...
[ -z "$OPENCLOUD_APPNAME_ENABLED" ] || if [ "$OPENCLOUD_APPNAME_ENABLED" != "yes" ]; then export SERVICE_DISABLED="$SERVICE_NAME" && __script_exit 0; fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
#SERVICE_GROUP="opencloud" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Set password length
RANDOM_PASS_USER=""
RANDOM_PASS_ROOT=""
...
OC_DATA_DIR="${OC_DATA_DIR:-$DATA_DIR}"
OC_LOG_LEVEL="${OC_LOG_LEVEL:-info}"
OC_INSECURE="${OC_INSECURE:-true}"
PROXY_HTTP_ADDR="${PROXY_HTTP_ADDR:-0.0.0.0:9200}"
# Disable TLS - running behind reverse proxy
PROXY_TLS="${PROXY_TLS:-false}"
# Collaboration service (Collabora) - connect to local instance
COLLABORATION_WOPI_SRC="${COLLABORATION_WOPI_SRC:-http://localhost:9980}"
COLLABORATION_APP_ADDR="${COLLABORATION_APP_ADDR:-http://localhost:9980}"
# Search service (Tika) - connect to local instance
SEARCH_EXTRACTOR_TYPE="${SEARCH_EXTRACTOR_TYPE:-tika}"
SEARCH_EXTRACTOR_TIKA_TIKA_URL="${SEARCH_EXTRACTOR_TIKA_TIKA_URL:-http://localhost:9998}"
# Antivirus service (ClamAV) - connect to local socket
ANTIVIRUS_SCANNER_TYPE="${ANTIVIRUS_SCANNER_TYPE:-clamav}"
ANTIVIRUS_CLAMAV_SOCKET="${ANTIVIRUS_CLAMAV_SOCKET:-/run/clamav/clamd.sock}"
# Notifications
...
CMD_ENV="OC_CONFIG_DIR=\"$OC_CONFIG_DIR\",OC_DATA_DIR=\"$OC_DATA_DIR\",OC_LOG_LEVEL=\"$OC_LOG_LEVEL\",OC_INSECURE=\"$OC_INSECURE\",PROXY_HTTP_ADDR=\"$PROXY_HTTP_ADDR\",PROXY_TLS=\"$PROXY_TLS\",SEARCH_EXTRACTOR_TYPE=\"$SEARCH_EXTRACTOR_TYPE\",SEARCH_EXTRACTOR_TIKA_TIKA_URL=\"$SEARCH_EXTRACTOR_TIKA_TIKA_URL\",ANTIVIRUS_SCANNER_TYPE=\"$ANTIVIRUS_SCANNER_TYPE\",ANTIVIRUS_CLAMAV_SOCKET=\"$ANTIVIRUS_CLAMAV_SOCKET\""
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
__execute_prerun() {
  ...
  # Set OpenCloud environment variables
  export OC_CONFIG_DIR="${OC_CONFIG_DIR:-$CONF_DIR}"
  export OC_DATA_DIR="${OC_DATA_DIR:-$DATA_DIR}"
  ...
  # Wait for supporting services to start (they run in separate init scripts)
  sleep 3

  # Initialize OpenCloud on first run
  if [ ! -f "$CONF_DIR/.initialized" ]; then
    echo "Initializing OpenCloud for first run..."

    # Set domain/URL if provided
    if [ -n "${OC_DOMAIN}" ]; then
      export OC_URL="${OC_URL:-https://${OC_DOMAIN}}"
    fi

    # Generate admin password if not set
    if [ -z "$ADMIN_PASSWORD" ]; then
      ADMIN_PASSWORD="$(__random_password 16)"
      echo "Generated admin password: $ADMIN_PASSWORD"
      echo "$ADMIN_PASSWORD" > "${ROOT_FILE_PREFIX}/opencloud_admin_pass"
    fi
    export ADMIN_PASSWORD

    # Run opencloud init with admin password
    if opencloud init --config-path "$CONF_DIR" --admin-password "$ADMIN_PASSWORD"; then
      touch "$CONF_DIR/.initialized"
      echo "OpenCloud initialized successfully"
    else
      echo "Failed to initialize OpenCloud" >&2
      return 1
    fi
  fi
...
__update_conf_files() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # replace variables
  # __replace "" "" "$CONF_DIR/opencloud.conf"
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$OPENCLOUD_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$OPENCLOUD_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$OPENCLOUD_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$OPENCLOUD_USER_PASS_WORD}"             #
...
__run_start_script() {
  ...
  #
  if [ -z "$cmd" ]; then
    __post_execute 2>"/dev/stderr" | tee -p -a "/data/logs/init.txt" || true
    retVal=$?
...
__run_secure_function() {
  ...
  if [ -n "$user_name" ] || [ -n "$user_pass" ]; then
    ...
    done 2>/dev/null | tee -p -a "/data/logs/init.txt" || true
  fi
...
for config_2_etc in $CONF_DIR $ADDITIONAL_CONFIG_DIRS; do
  __initialize_system_etc "$config_2_etc" 2>/dev/stderr | tee -p -a "/data/logs/init.txt" || true
done
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Replace variables
