...
# - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202511211130-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  05-collabora.sh --help
# @@Copyright        :  Copyright: (c) 2025 Jason Hempstead, Casjays Developments
# @@Created          :  Friday, Nov 21, 2025 11:30 EST
# @@File             :  05-collabora.sh
# @@Description      :  
...
[ -z "$COLLABORA_APPNAME_ENABLED" ] || if [ "$COLLABORA_APPNAME_ENABLED" != "yes" ]; then export SERVICE_DISABLED="$SERVICE_NAME" && __script_exit 0; fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
__execute_prerun() {
  ...
  # Set COLLABORA_SERVER_NAME from DOMAIN or HOSTNAME (subdomain format)
  local base_domain="${DOMAIN:-${HOSTNAME:-localhost}}"
  export COLLABORA_SERVER_NAME="${COLLABORA_SERVER_NAME:-collabora.${base_domain}}"

  # Add internal hostname resolution for all-in-one container
  # This allows services to reach each other via subdomain names internally
  if ! grep -q "collabora.${base_domain}" /etc/hosts 2>/dev/null; then
    echo "127.0.0.1 ${base_domain} collabora.${base_domain} radicale.${base_domain}" >> /etc/hosts
  fi

  # Create necessary directories for Collabora
  mkdir -p /var/log/coolwsd /tmp/coolwsd
  mkdir -p /opt/cool/child-roots
  chown -R cool:cool /var/log/coolwsd /tmp/coolwsd /opt/cool 2>/dev/null || true

  # Ensure coolwsd config directory exists
  mkdir -p /etc/coolwsd
  chown -R cool:cool /etc/coolwsd 2>/dev/null || true
...
__update_conf_files() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - -
  # replace variables
  # __replace "" "" "$CONF_DIR/collabora.conf"
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$COLLABORA_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$COLLABORA_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$COLLABORA_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$COLLABORA_USER_PASS_WORD}"             #
