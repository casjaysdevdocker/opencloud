...
# @@Created          :  Friday, Nov 21, 2025 10:36 EST
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for opencloud
...
SERVICE_USER="${SERVICE_USER:-opencloud}"   # execute command as another user
SERVICE_GROUP="${SERVICE_GROUP:-opencloud}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for opencloud"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
